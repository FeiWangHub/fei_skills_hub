# Fei Skills Hub — AGENTS.md

> **Note:** This file provides context for Codex (and other AI agents) working in this repository. For human-readable project docs, see [`README.md`](./README.md).
...
- **Tool-agnostic:** Works with Codex, GitHub Copilot (VS Code / IntelliJ), Gemini CLI, Cursor, Windsurf, OpenCode
- **License:** MIT
...
├── .Codex/                    # Codex local config
│   └── settings.local.json     # Codex-specific permissions
├── .github/
...
- **Tool-agnostic:** Skills use Markdown-based `skill.md` files, not tool-specific formats. They should work across Codex, Copilot, Gemini CLI, etc.
...
The `.Codex/settings.local.json` file contains Codex-specific permissions (currently allows `WebSearch`, `git add`, `gh api`, and `git commit` commands). This file is gitignored and should not be committed.
